...
for i in "${!SCENES[@]}"; do
  ...
  echo "Getting language-prompted planar priors of ${SCENE} on GPU ${GPU}"

  CUDA_VISIBLE_DEVICES=${GPU} \
  python run_lp3.py -s "${DATASET_BASE}/${SCENE}" \
    -t "wall. floor. door. screen. window. ceiling. table" --vis &
